...
import { Activity, Droplets, Crosshair, Loader2, Sparkles, RefreshCw } from 'lucide-react';
import api from '../../services/api';

export const SnakeWaterGun = () => {
  ...
  const choices = [
    { id: 'snake', name: 'Snake', icon: <Activity className="h-10 w-10" />, color: 'hover:border-green-500/50 hover:bg-green-500/10 hover:text-green-400' },
    { id: 'water', name: 'Water', icon: <Droplets className="h-10 w-10" />, color: 'hover:border-blue-500/50 hover:bg-blue-500/10 hover:text-blue-400' },
    { id: 'gun', name: 'Gun', icon: <Crosshair className="h-10 w-10" />, color: 'hover:border-slate-500/50 hover:bg-slate-500/10 hover:text-slate-400' },
  ];
  ...
  const handlePlay = async (choice) => {
    ...
    setAnimationText('Snake...');
    setTimeout(() => setAnimationText('Water...'), 750);
    setTimeout(() => setAnimationText('Gun...'), 1500);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text text-transparent inline-block">
          Snake Water Gun
        </h1>
        <p className="text-slate-400">Can you survive the wild against the AI?</p>
      </div>
              ...
              <div className="flex flex-wrap justify-center gap-6">
                {choices.map((choice) => (
                  <button
                    ...
                    disabled={loading}
                    className={`group relative flex h-40 w-40 flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dark-700 bg-dark-900/50 p-6 text-slate-400 transition-all ${choice.color} disabled:opacity-50 disabled:cursor-not-allowed`}
                  >
                    <div className="transition-transform group-hover:scale-110 group-hover:-translate-y-2">
                       {choice.icon}
                    </div>
